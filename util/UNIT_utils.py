...
from data.image_folder import ImageFilelist, ImageFolder
import torch
import os
import math
...
def get_all_data_loaders(args):
    batch_size = args.batch_size
    num_workers = args.num_threads
    new_size_a = new_size_b = args.load_size
    height = args.crop_size
    width = args.crop_size

    train_loader_a = get_data_loader_folder(os.path.join(args.dataroot, 'trainA'), batch_size, True,
                                            new_size_a, height, width, num_workers, True)
    test_loader_a = get_data_loader_folder(os.path.join(args.dataroot, 'testA'), batch_size, False,
                                            new_size_a, new_size_a, new_size_a, num_workers, True)
    train_loader_b = get_data_loader_folder(os.path.join(args.dataroot, 'trainB'), batch_size, True,
                                            new_size_b, height, width, num_workers, True)
    test_loader_b = get_data_loader_folder(os.path.join(args.dataroot, 'testB'), batch_size, False,
...
def prepare_sub_folder(output_directory):
    ...
    checkpoint_directory = os.path.join(output_directory, 'model')
...
def get_scheduler(optimizer, args, iterations=-1):
    if args.lr_policy== 'constant':
        ...
        scheduler = lr_scheduler.StepLR(optimizer, step_size=args.lr_decay_iters,
                                        gamma=args.gamma, last_epoch=iterations)
    else:
