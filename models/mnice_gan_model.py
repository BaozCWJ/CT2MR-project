...
class mNICE(object) :
    ...
    def __init__(self, args):
        ...
        if self.light :
            self.model_name = 'mNICE_light'
        else :
            self.model_name = 'mNICE'
    ...
    def train(self):
        loss_A = [] #D_loss, G_loss, ad_loss, cycle_loss, recon_loss
        loss_B = []
        ...
        for step in range(self.start_iter, self.iteration + 1):
            ...
            real_A, real_B, label = real_A.to(self.device), real_B.to(self.device), label.to(self.device)
            ...
            G_pair_loss_B = self.L1_loss(label*fake_A2B,label*real_B)
            G_pair_loss_A = self.L1_loss(fake_B2A,real_A)
            ...
            G_cycle_loss_B = self.L1_loss(label*fake_B2A2B, label*real_B)
            ...
            G_loss_A = self.adv_weight * (G_ad_loss_GA + G_ad_cam_loss_A + G_ad_loss_LA ) + self.cycle_weight * G_cycle_loss_A + self.recon_weight * G_recon_loss_A + self.pair_weight *G_pair_loss_B
            G_loss_B = self.adv_weight * (G_ad_loss_GB + G_ad_cam_loss_B + G_ad_loss_LB ) + self.cycle_weight * G_cycle_loss_B + self.recon_weight * G_recon_loss_B + self.pair_weight *G_pair_loss_A
